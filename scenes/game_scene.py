...
class GameScene(BaseScene):
    def __init__(self, sm):
        ...
        self.correct_answers_text = Text(w // 2 + 250, h - 560, 0, 0, sm,
                                         f"Correct answers: {sm.correct_count}{self.addition}")
        self.timer_text = Text(w // 2, h - 560, 0, 0, sm, str(sm.time_left))
        print(f"Initiated color {self.scene_manager.themes['text-color']}")
        self.question_count_text = Text(w // 2 - 250, h - 560, 0, 0, sm,
                                        f"Question: {sm.question_count}/{sm.max_questions}")
        self.question_text = Text(w // 2, h - 420, 0, 0, sm, self.selected.question, 25)
        ...
        self.ui_list.extend(
            [self.question_text, self.answer_one_button, self.answer_two_button, self.answer_three_button,
    ...
    def next_question(self):
        ...
        update_view(self)  # Update the view and set all graphics to initial values
        self.timer_text.color = self.scene_manager.themes['text-color']
